...
public abstract class VkApiMethod<Response> {
    ...
    public Response execute() throws VkApiException {
        ...
        //LogMaster.log("Sending: method={" + getMethod() + "}, url={" + getUrl() + "}, params={" + stringParams + "}");
        ...
        try {
            ...
            while(ist) {
                try {
                    body = vkApiHttpClient.execute();
                    ist = false;
                } catch (IOException e) {
                    LogMaster.log("[WARNING]: Internet connection problems. I'm waiting 5 seconds...");
                    counter++;
                    Thread.sleep(5000);
                }
            }
            if(counter > 0)
                LogMaster.log("[KEEP WORKING]: The Internet connection has appeared! The bot continues to work...");
